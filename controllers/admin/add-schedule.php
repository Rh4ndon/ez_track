...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $schedule = getRecord('schedules', 'teacher_id = "' . $teacher . '" AND subject_id = "' . $subject . '" AND section_id = "' . $section . '" AND day_of_week = "' . $day . '" AND start_time = "' . $start_time . '" AND end_time = "' . $end_time . '"');
    if ($schedule) {
        header('Content-Type: application/json');
    ...
    $subjectConflict = getRecord(
        'schedules',
        'section_id = "' . $section . '" AND subject_id = "' . $subject . '" AND day_of_week = "' . $day . '"'
    );
    ...
    // Insert the record
    $result = insertRecord('schedules', $data);
    ...
    if ($result) {
        ...
        header('Content-Type: application/json');
        echo json_encode(array('error' => 'Failed to insert schedule.'));
    }
} else {
    header('Content-Type: application/json');
